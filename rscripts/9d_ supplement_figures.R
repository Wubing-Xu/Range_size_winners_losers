...
plot_dynamic <- ggplot(species_dynamic_longer, aes( x = study_id, y = number, fill = dynamic)) + 
  ...
  scale_x_continuous(expand = c(0.01,0.01), breaks = c(1, 40, 80, 120, 160, 200, 238), labels = c(1, 40, 80, 120, 160, 200, 238)) +
  theme_bw() +
...
length(which(nocc$ratio_nocc_gbif>=1))/nrow(nocc)  # 95.6%
table(nocc$ratio_nocc_gbif>=5) 
length(which(nocc$ratio_nocc_gbif>=5))/nrow(nocc) # 83.3%
...
lm1 <- lm(estimate_slope ~ 0 + realm + Psprich_range:realm, data = brm_oc_aoo10_coef)
summary(lm1)
...
cowplot::plot_grid(hist_psprich_range, plot_slope_psprich_range, 
                   nrow = 1, align = "hv")
ggsave("results/Fig.S_psprich_range.png", width = 140, height = 70, units = 'mm', dpi = 600)
...
ggplot(occupancy_first_last) +
  ...
  labs(x = "Occupancy in the early period",
       y = "Occupancy in the late period",
...
plot_taxon_fixef <- ggplot(brm_oc_aoo10_realm_taxa_fixed) +
  ...
  geom_text(aes(y = taxon_new, x = 0.20, label = paste('n == ', nstudy), group = realm), 
...
plot_region_fixef <- ggplot(brm_oc_aoo10_realm_region_fixed %>% filter(term == "slope")) +
  ...
  geom_point(aes(y = region, x = Estimate, colour = realm),
             position = position_dodge(0.7), size = 2) + 
    geom_text(aes(y = region, x = 0.20, label = paste('n == ', nstudy), group = realm), 
            position = position_dodge(0.7), parse = T, size = 2.1, hjust = 0) +
  geom_vline(xintercept = 0, linetype = 2, size = 0.3) + 
  scale_x_continuous(limits = c(xlimit_low, 1.3*xlimit_high)) +
  labs(x = "Effect of range size", ylab = NULL, colour = NULL) + 
...
plot_oc_aoo_taxa <- ggplot() + 
  ...
  geom_ribbon(data = brm_oc_aoo10_realm_taxa_fitted,
              aes(x = aoo10, ymin = oc_sqroot_Q2.5, ymax = oc_sqroot_Q97.5, fill = taxon_new), 
              size =1, alpha = 0.5) + 
...
plot_oc_aoo_region <- ggplot() + 
  ...
  geom_ribbon(data = brm_oc_aoo10_realm_region_fitted,
              aes(x = aoo10, ymin = oc_sqroot_Q2.5, ymax = oc_sqroot_Q97.5, fill = region), 
              size =1, alpha = 0.3) + 
