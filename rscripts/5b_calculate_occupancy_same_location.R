## For sensitive analyses: use the assemblage data with same locations across years to calculate occupancy
...
dat_sloc_occupancy_period <- dat_sloc %>% 
  left_join(years_period, by = c("study", "year")) %>% 
  # mutate(sample = paste(year, sample, sep="_")) %>% 
...
# calculate occupancy
occupancy_sloc <- get_occupancy(data = dat_sloc_occupancy_period,
                           fillspecies = TRUE, trimsamples = FALSE, occ_full = FALSE, occ_rarefy = TRUE, resamples = 1)

occupancy_sloc <- occupancy_sloc %>% 
  rename(specieskey = species) %>%
  left_join(dat_sloc %>% distinct(study, database, studyID), by =c("study")) %>% 
  left_join(years_period, by = c("study", "year")) %>%
  left_join(dat_sloc %>% distinct(species, specieskey), by =c("specieskey")) %>%
...
mean_occupancy_sloc_period  <- occupancy_sloc  %>% 
  group_by(resample, study, specieskey, period_combined) %>%
  summarise(n_occ = sum(n_occ),
            occupancy = mean(occupancy)) %>%
  ungroup()

occupancy_sloc_period <- occupancy_sloc %>% 
  dplyr::select(-c(n_samp, n_occ, occupancy)) %>% 
  distinct() %>% 
  left_join(mean_occupancy_sloc_period, by = c("resample","study", "specieskey", "period_combined"))


## calculate occupancy change
# nest different years of the same species
occupancy_sloc_period_nest <- occupancy_sloc_period %>% 
  dplyr::select(-period_combined, - year_mean_period) %>%
  group_by(resample, study, species) %>%
  nest(data = c(year_period, n_occ, occupancy)) %>%
  ungroup()

occupancy_change_sloc_period <- tibble()
for(i in 1:nrow(occupancy_sloc_period_nest)){
  res <- get_occup_change(x = occupancy_sloc_period_nest[i,]$data[[1]])
  occupancy_change_sloc_period <- bind_rows(occupancy_change_sloc_period, res)
}

occupancy_change_sloc_period <- occupancy_sloc_period_nest %>% 
  dplyr::select(-data) %>%
